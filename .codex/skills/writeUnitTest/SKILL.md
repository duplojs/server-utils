...
La fonction testée est toujours appelée via son **namespace** (ex. `DServerFile.copy`).

## Entrées (ce qu’il faut déduire de la tâche ou du codebase)
Le prompt utilisateur fournira généralement :
- Namespace et nom de fonction : ex. `DServerFile.copy`
...
- `scripts/file/copy.ts` → `tests/file/copy.test.ts`
...
Exemple :
```ts
import { type ExpectType, pipe } "@duplojs/utils";
import { DServerFile } from "@scripts";
```
