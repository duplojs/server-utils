import { innerPipe, when, pipe } from '@duplojs/utils';
...
function parseEnvironmentLine(line) {
    return pipe(line, SS.replace(endLineBreakerRegex, "\n"), SS.extractAll(lineRegex), GG.reduce(GG.reduceFrom({}), ({ element, nextWithObject, lastValue, next }) => {
            ...
            return nextWithObject(lastValue, {
                [element.namedGroups.key]: pipe(element.namedGroups.value, (value) => {
                    ...
                    if (SS.startsWith(value, "\"")) {
                        return pipe(surroundingValue, SS.replace(newLineRegex, "\n"), SS.replace(backCartRegex, "\r"));
                    }
...
function parseEnvironmentFiles(baseEnv, paths) {
    return GG.asyncReduce(paths, GG.reduceFrom([baseEnv]), ({ lastValue, element, nextPush, exit }) => readTextFile(element)
        .then(innerPipe(EE.whenIsRight(innerPipe(parseEnvironmentLine, (value) => nextPush(lastValue, value))), when(EE.isLeft, exit))));
}
