...
describe("initOption", () => {
	...
	it("throws when required value looks like another option", () => {
		...
		expect(() => option.execute(["--port", "--other"])).toThrowError(DServerCommand.CommandOptionValueLooksLikeOptionError);
	});
	...
	it("throws when value is provided to an option without value", () => {
		...
		expect(() => option.execute(["--verbose=true"])).toThrowError(DServerCommand.CommandOptionValueNotRequiredError);
	});
